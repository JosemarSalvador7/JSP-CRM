...
from accounts.forms import FormLogin , RegisterUserForm
...
def list_view(requests):
    ...
    return render(requests, "list_accounts.html", {"users": users,  'total_users': User.objects.count(),
    'sellers_count': User.objects.filter(profile__role='V').count(),
    'managers_count': User.objects.filter(profile__role='G').count(),
    'users_with_avatar': User.objects.filter(profile__avatar__isnull=False).count(),
    })
