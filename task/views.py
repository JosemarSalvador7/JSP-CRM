...
def gerar_pdf_tasks(request):
    ...
    for task in tasks:
        ...
        # Data de Vencimento
        pdf.cell(
            w=35, h=8, txt=task.due_date.strftime("%d/%m/%Y"), border=1, ln=0, align="c"
        )
        ...
        # Contato
        contato = (
            f"{task.contact.name} {task.contact.surname}"
            if task.contact
            else "Sem contato"
        )
    ...
    return FileResponse(
        BytesIO(pdf_out), filename=f"tarefas_{request.user.username}.pdf"
    )
