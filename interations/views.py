...
def list_view(request):
    interactions = (
        Interaction.objects.filter(created_by=request.user)
...
def gerar_pdf_interactions(request):
    ...
    interactions = Interaction.objects.filter(created_by=request.user).select_related(
        "contact", "created_by"
    )
    ...
    for interaction in interactions:
        # Tipo
        tipo_display = dict(Interaction.interaction_choices).get(
        ...
        pdf.cell(
            w=40,
            h=8,
            txt=interaction.date_time.strftime("%d/%m/%Y %H:%M"),
            border=1,
            ln=0,
            align="c",
        )
        ...
        # Descrição (truncada)
        descricao = interaction.description[:60] + (
            "..." if len(interaction.description) > 60 else ""
        )
        pdf.cell(w=80, h=8, txt=descricao, border=1, ln=0, align="l")

        # Criado por
        criado_por = (
            interaction.created_by.get_full_name() or interaction.created_by.username
        )
    ...
    pdf.cell(
        w=0, h=10, txt=f"Total de interações: {interactions.count()}", ln=1, align="R"
    )

    pdf_out = pdf.output(dest="S").encode("latin1")
    return FileResponse(
        BytesIO(pdf_out), filename=f"interacoes_{request.user.username}.pdf"
    )
...
def retrievepdf_interaction(request, id):
    ...
    pdf.cell(
        w=10,
        h=10,
        txt=interaction.date_time.strftime("%d/%m/%Y às %H:%M"),
        ln=1,
        align="c",
    )
    pdf.cell(w=10, h=5, txt="", ln=1, align="c")
    ...
    pdf_out = pdf.output(dest="S").encode("latin1")
    nome_arquivo = (
        f"interacao_{interaction.contact.name}_{interaction.contact.surname}.pdf"
    )
